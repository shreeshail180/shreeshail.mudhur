CREATE OR REPLACE PROCEDURE "MERGE_INCREMENTAL_SP_DIGITAL_BULK_V2_1"("OPTIONS" VARCHAR(16777216))
...
var update_join_rs = update_join_sql_stmt.execute();
update_join_rs.next(); 
update_key_statement = update_join_rs.getColumnValue(1); 

result = result + " 3.build join conditions for "+target_tables+ "\\n"

// Build set statement
update_set_str = "SELECT distinct listagg( distinct concat(target_entity_nm,''.'',target_column_nm,''"
                +" = src.'',target_column_nm) ,'', \\n'')"
                +" FROM "+ metadata_db_schema +"."+"source_to_target_attribute_mapping att_rel \\n"
                +" JOIN "+ metadata_db_schema +"."+"attribute_details att \\n" 
                +" ON att_rel.target_entity_key = att.entity_key \\n"
                //+" AND upper(trim(att_rel.target_column_nm)) = upper(trim(att.attribute_nm)) \\n"
                +" WHERE UPPER(source_entity_name)  in ("+ src_table_name +" ) \\n"
                +" AND UPPER(target_entity_nm) = UPPER(''"+ target_tables +"'') \\n"					
                +" AND UPPER(skip_update) = ''N'' \\n"
                +" AND NVL(UPPER(is_pk),''N'') = ''N'' ;\\n"
var update_set_sql_stmt = snowflake.createStatement(
 {
 sqlText: update_set_str
 }
);  
